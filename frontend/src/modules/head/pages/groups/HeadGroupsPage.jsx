...
import { 
  Users, CheckCircle2, Search, Plus, 
  Eye, ShieldCheck, Grid, List, 
  AlertCircle, ChevronDown, Download
} from 'lucide-react';
import { headGroupApi } from '../../services/headGroupApi';
import { Avatar } from '../../../member/components/common/Avatar';

export const HeadGroupsPage = () => {
  ...
  const fetchGroups = async () => {
    try {
      setLoading(true);
      const res = await headGroupApi.getGroups();
      ...
      console.error(err);
      // Mock data fallback if backend is not fully ready
      const mockData = [
        { _id: '1', name: 'Youth Committee', description: 'Organizing youth events', category: 'Social', type: 'public', approvalStatus: 'approved', memberCount: 15, adminCount: 2, isArchived: false, createdAt: new Date().toISOString() },
        { _id: '2', name: 'Business Network', description: 'Networking for Samaj business owners', category: 'Professional', type: 'invite_only', approvalStatus: 'pending', memberCount: 1, adminCount: 1, isArchived: false, createdAt: new Date().toISOString() },
      ];
      setGroups(mockData);
  ...
  const handleApprove = async (id, name) => {
    try {
      await headGroupApi.updateGroupStatus(id, 'approved');
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Manage and moderate all groups in your community</p>
            </div>
          </div>

          <div className="flex flex-wrap items-center gap-2.5">
            <button 
              onClick={() => showToast('Create Group flow to be implemented')}
                ...
                <tr key={group._id} className="hover:bg-slate-50/40 transition-all">
                  <td className="p-3.5 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400">{group.type === 'invite_only' ? 'Private' : 'Public'}</p>
                    </div>
                  </td>
                  <td className="p-3.5">{group.category || 'General'}</td>
