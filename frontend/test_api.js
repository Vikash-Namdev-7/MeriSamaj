...
const USER1 = { email: process.env.USER1_EMAIL || 'test1@merisamaj.com', password: process.env.USER1_PASS || 'Test@123' };
const USER2 = { email: process.env.USER2_EMAIL || 'test2@merisamaj.com', password: process.env.USER2_PASS || 'Test@123' };
...
async function run() {
  ...
  await test('Login User 1', async () => {
    ...
    token1 = res.data?.data?.token || res.data?.token;
    userId1 = res.data?.data?.user?._id || res.data?.data?.member?._id;
  ...
  await test('Login User 2', async () => {
    ...
    token2 = res.data?.data?.token || res.data?.token;
    userId2 = res.data?.data?.user?._id || res.data?.data?.member?._id;
  ...
  await test('POST /member/groups/:id/messages — send message', async () => {
    if (!groupId) return 'SKIP';
    const res = await request('POST', `/member/groups/${groupId}/messages`, {
  ...
  await test('GET /member/groups/:id/messages — get messages', async () => {
    if (!groupId) return 'SKIP';
    const res = await request('GET', `/member/groups/${groupId}/messages?page=1&limit=20`, null, token1);
    assertStatus(res, 200);
  ...
  await test('PUT /member/groups/messages/:msgId/pin — pin message', async () => {
    if (!groupMsgId) return 'SKIP';
    const res = await request('PUT', `/member/groups/messages/${groupMsgId}/pin`, null, token1);
    assertStatus(res, 200);
    return { isPinned: res.data?.data?.message?.isPinned };
  });
  ...
  await test('POST /member/chat/open — open conversation', async () => {
    if (!token1 || !userId2) return 'SKIP';
    const res = await request('POST', '/member/chat/open', { targetUserId: userId2 }, token1);
    assertStatus(res, 200);
  ...
  await test('POST /member/chat/:convId/messages — send message', async () => {
    if (!convId) return 'SKIP';
    const res = await request('POST', `/member/chat/${convId}/messages`, {
      message: 'Hello from smoke test',
      type: 'text'
    }, token1);
    assertStatus(res, 201);
  ...
  await test('GET /member/chat/:convId/messages — get messages', async () => {
    if (!convId) return 'SKIP';
    const res = await request('GET', `/member/chat/${convId}/messages?page=1&limit=20`, null, token1);
    assertStatus(res, 200);
    const msgs = res.data?.data?.messages;
    assert(Array.isArray(msgs), 'messages not array');
    return { count: msgs.length };
  ...
  await test('POST /member/chat/:convId/messages/seen — mark seen', async () => {
    if (!convId || !memberMsgId) return 'SKIP';
    const res = await request('POST', `/member/chat/${convId}/messages/seen`, {
  ...
  await test('GET /member/announcements/channels — list channels', async () => {
    const res = await request('GET', '/member/announcements/channels', null, token1);
    assertStatus(res, 200);
  ...
  await test('POST /member/announcements/:channelId — post announcement', async () => {
    if (!channelId) return 'SKIP';
    const res = await request('POST', `/member/announcements/${channelId}`, {
      message: 'Test announcement from smoke test',
      type: 'text'
    }, token1);
    // 201 = success, 403 = no permission (user is not head/admin)
    if (res.status === 403) {
      return 'SKIP'; // Permission check working correctly
    }
    assertStatus(res, 201);
  ...
  await test('WRONG USER: User2 cannot delete User1 message', async () => {
    // Re-send a message from User1
    if (!convId || !token2) return 'SKIP';
    const sendRes = await request('POST', `/member/chat/${convId}/messages`, {
    ...
    assert([403, 401].includes(delRes.status), `Expected 403, got ${delRes.status} (User2 should NOT be able to delete User1's messages for everyone)`);
    return { statusCode: delRes.status };
  });

  await test('COMMUNITY ISOLATION: Non-member group access', async () => {
    // This is hard to test without a second community user, just verify 
    // the group detail endpoint validates membership
    if (!groupId || !token2) return 'SKIP';
    // User2 (who left the group) tries to post to the group
    const res = await request('POST', `/member/groups/${groupId}/messages`, {
